...
const AUDIENCE = [
  {
    icon: Target,
    text: 'Competitive triathletes targeting age-group podiums or Ironman qualification',
  },
  {
    icon: Zap,
    text: 'Distance runners chasing personal bests from 5K through ultra-marathon',
  },
  {
    icon: Users,
    text: 'Multi-sport athletes transitioning into triathlon from a single-sport background',
  },
  {
    icon: HeartPulse,
    text: 'Dedicated age-groupers who want structure, accountability, and expert guidance',
  },
];
...
const DELIVERABLES = [
  {
    icon: ClipboardList,
    text: 'Fully individualized daily training plans delivered to your platform of choice',
  },
  {
    icon: MessageCircle,
    text: 'Weekly one-on-one check-ins to review progress, adjust plans, and set priorities',
  },
  {
    icon: BarChart3,
    text: 'Ongoing data analysis of workouts with actionable feedback on every key session',
  },
  {
    icon: CalendarCheck,
    text: 'Race-day strategy and pacing plans tailored to course profile and conditions',
  },
  {
    icon: Target,
    text: 'Season planning and goal-setting with clear milestone checkpoints',
  },
  {
    icon: HeartPulse,
    text: 'Guidance on recovery protocols, nutrition timing, and training-life balance',
  },
];
...
const FAQ_ITEMS = [
  {
    ...
    answer: '{/* DRAFT: Client to provide detailed answers */}',
  },
  {
  ...
  return (
    <main>
      <Helmet>
        ...
        <meta name="description" content="1-on-1 endurance coaching for competitive triathletes and runners. Personalized training plans, data analysis, race strategy." />
      </Helmet>
      {/* ─── Hero ─── */}
      <PageHero
        title="High-Performance Coaching"
        subtitle="1-on-1 endurance coaching for competitive triathletes and runners who refuse to settle for average."
        breadcrumb="Programs / Coaching"
      />
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {METHODOLOGY.map((item) => (
              ...
              >
                ...
                <h3 className="font-display text-xl mb-3 text-[#1A1A1A]">
                  {item.title}
                </h3>
      ...
      <section ref={faqRef} className="section-spacing bg-[#F8F8F8]">
        <div className="max-w-3xl mx-auto px-6">
          <p className="font-mono-label text-sm text-[#C41E3A] mb-3 text-center">
            Common Questions
          </p>
              ...
              {FAQ_ITEMS.map((item, index) => (
                <AccordionItem key={index} value={`faq-${index}`}>
                  ...
                  <AccordionContent className="text-[#4A4A4A] leading-relaxed">
                    {/* DRAFT: Client to provide detailed answers */}
                    Answer coming soon.
                  </AccordionContent>
                </AccordionItem>
              ))}
      ...
      <CTABanner
        ...
        subheading="Apply for a coaching spot and let's build your best season yet."
        variant="apply"
      />
    </main>
  );
